...
export default class AwardRecipient implements ISerializable {
  ...
  toJSON(): object {
    return {
      ...
      award: this.award.awardKey.length > 0 ? this.award.toJSON() : new Award().toJSON(),
      team: this.team.teamKey.length > 0 ? this.team.toJSON() : new Team().toJSON()
    };
  }
  ...
  fromJSON(json: any): AwardRecipient {
    ...
    award.award =
      typeof json.award !== "undefined"
        ? new Award().fromJSON(json.award)
        : new Award();
    award.team =
      typeof json.team !== "undefined"
        ? new Team().fromJSON(json.team)
        : new Team();
    return award;
  }
